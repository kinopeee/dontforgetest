---
name: テスト生成エージェント拡張機能
overview: Cursor CLI（cursor-agent）をヘッドレスモードで非同期呼び出しし、コミット差分や選択範囲からテストコードを自動生成するVS Code拡張機能を実装する。
todos:
  # === MVP (Phase 1) ===
  - id: mvp-types
    content: "[MVP] 共通型定義（event.ts: TestGenEvent, provider.ts: Provider interface）"
    status: completed
  - id: mvp-output-channel
    content: "[MVP] Output Channel 管理モジュール（ui/outputChannel.ts）"
    status: completed
  - id: mvp-preflight
    content: "[MVP] プリフライトチェック（CLI存在確認、設定検証）"
    status: completed
  - id: mvp-cursor-provider
    content: "[MVP] cursor-agent Provider実装（spawn + stream-json パース）"
    status: completed
  - id: mvp-prompt-builder
    content: "[MVP] プロンプト構築（test-strategy.md読み込み + テンプレート組み立て）"
    status: completed
  - id: mvp-generate-cmd
    content: "[MVP] テスト生成コマンド実装（generateTestFromCommit）"
    status: completed
  # === Phase 2: Git連携 ===
  - id: phase2-git-diff
    content: git diff 解析モジュール（git/diffAnalyzer.ts）
    status: completed
  - id: phase2-commit-range
    content: コミット範囲指定コマンド（generateTestFromCommitRange）
    status: completed
  - id: phase2-working-tree
    content: "未コミット差分コマンド（generateTestFromWorkingTree: staged/unstaged）"
    status: completed
  # === Phase 3: UX改善 ===
  - id: phase3-statusbar
    content: ステータスバー進捗表示（ui/statusBar.ts）
    status: completed
  - id: phase3-quickpick
    content: QuickPickでソース/モデル選択UI（ui/quickPick.ts）
    status: completed
  - id: phase3-model-settings
    content: モデル設定（設定画面 + customModelsフォールバック）
    status: completed
  # === Phase 4: 高度な機能 ===
  - id: phase4-webview-panel
    content: TestGen操作パネル（ui/controlPanel.ts - WebviewView）
    status: completed
  - id: phase4-model-fetch
    content: "[中止] Cursor API (GET /v0/models) からモデルリスト動的取得（設定→自動選択フォールバックに統一）"
    status: cancelled
  - id: phase4-worktree-sandbox
    content: "[任意] git worktree サンドボックス実行"
    status: pending
  - id: phase4-artifact-perspective-table
    content: テスト生成前にテスト観点表を生成してファイル保存（タイムスタンプ付き）
    status: completed
  - id: phase4-artifact-test-exec-report
    content: テスト生成後にテストを実行し、テスト実行レポートをファイル保存（タイムスタンプ付き）
    status: completed
  # === Phase 4 追加実装（プラン外で実装済み） ===
  - id: phase4-artifacts-module
    content: 成果物管理モジュール（core/artifacts.ts - 観点表・実行レポートの生成・保存・検索）
    status: completed
  - id: phase4-default-test-strategy
    content: 内蔵デフォルトテスト戦略（core/defaultTestStrategy.ts - 英語版ルール）
    status: completed
  - id: phase4-test-result-parser
    content: テスト結果パーサー（core/testResultParser.ts - Mocha形式出力解析）
    status: completed
  - id: phase4-test-runner
    content: テストコマンド実行エンジン（core/testRunner.ts）
    status: completed
  - id: phase4-run-with-artifacts
    content: 成果物保存付き実行ラッパー（commands/runWithArtifacts.ts）
    status: completed
  - id: phase4-select-default-model
    content: デフォルトモデル選択コマンド（commands/selectDefaultModel.ts）
    status: completed
  - id: phase4-open-artifacts-cmd
    content: 最新の観点表・実行レポートを開くコマンド
    status: completed
  - id: phase4-test-execution-runner-setting
    content: テスト実行担当者設定（cursorAgent/extension切り替え）
    status: completed
  - id: phase4-vscode-test-detection
    content: VS Code起動テストの自動検出・スキップ機能
    status: completed
  - id: phase4-cursor-agent-fallback
    content: cursor-agentコマンド実行拒否時の拡張機能側フォールバック
    status: completed
  # === Phase 5: カバレッジ連携（将来） ===
  - id: phase5-coverage-analyzer
    content: "[将来] カバレッジレポート解析（coverage/coverageAnalyzer.ts）"
    status: pending
  - id: phase5-coverage-cmd
    content: "[将来] カバレッジ不足箇所からのテスト生成コマンド"
    status: pending
  # === Phase 6: 品質評価（将来） ===
  - id: phase6-quality-eval
    content: "[将来] 生成テストの品質評価（実行・カバレッジ・モデル比較）"
    status: pending
  - id: phase6-prompt-tuning
    content: "[将来] 評価結果に基づくプロンプト改善"
    status: pending
---
...
| コマンドID | 機能 | Phase ||-----------|------|:-----:|| `testgen-agent.generateTest` | QuickPickでソース/モデル選択して実行 | MVP || `testgen-agent.generateTestFromCommit` | 最新コミットの変更に対するテスト生成 | MVP || `testgen-agent.showTestGeneratorOutput` | Output Channel を表示 | MVP || `testgen-agent.generateTestFromCommitRange` | 指定したコミット範囲の差分に対するテスト生成 | Phase 2 || `testgen-agent.generateTestFromWorkingTree` | 未コミット差分に対するテスト生成 | Phase 2 || `testgen-agent.selectDefaultModel` | デフォルトモデルを変更 | Phase 3 || `testgen-agent.previewLastRun` | 直近生成の変更差分を表示 | Phase 3 || `testgen-agent.rollbackLastRun` | 直近生成の変更を取り消す | Phase 3 || `testgen-agent.openTestStrategy` | テスト戦略ルールファイルを開く | Phase 3 || `testgen-agent.openPanel` | WebviewViewパネルを開く | Phase 4 || `testgen-agent.generateTestFromCoverage` | 未カバー行/関数に対するテスト生成 | Phase 5 |

## 設定項目 (contributes.configuration)

- `testgen-agent.defaultModel` (string): デフォルトで使用するモデル
- `testgen-agent.customModels` (string[]): モデル候補リスト（QuickPick等で使用）
...
- **Model**: 設定（defaultModel/customModels）から選択。未設定時は cursor-agent の自動選択にフォールバック（API動的取得は中止）
...
ユーザーが「生成の根拠」と「実行結果」を後から追跡できるよう、以下を **毎回新規ファイルとして保存**する。

### 1) テスト観点表（生成前）

- **タイミング**: テストコード生成を開始する前
- **内容**: Markdown形式のテスト観点表
- **ファイル名**: タイムスタンプ付きでユニークにする（例: `test-perspectives_20251223_014959.md`）
- **保存場所（デフォルト）**: ワークスペース配下 `docs/test-perspectives/`
  - 既存の `docs/test-perspectives.md` は「固定版（手書き/集約）」として残し、**自動生成物は別フォルダに蓄積**する方針

### 2) テスト実行レポート（生成後）

- **タイミング**: テスト生成（必要なら適用）後、テスト実行が完了した直後
- **内容**: 実行環境、実行コマンド、標準出力/標準エラー（必要なら抜粋/全文）、終了コード、所要時間など
- **ファイル名**: タイムスタンプ付きでユニークにする（例: `test-execution_20251223_015130.md`）
- **保存場所（デフォルト）**: ワークスペース配下 `docs/test-execution-reports/`

### タイムスタンプ形式（推奨）

- `YYYYMMDD_HHmmss`（例: `20251223_015130`）

### 設定項目（追加予定）

- `testgen-agent.perspectiveReportDir` (string): 観点表の保存先（デフォルト: `docs/test-perspectives`）
- `testgen-agent.testExecutionReportDir` (string): テスト実行レポートの保存先（デフォルト: `docs/test-execution-reports`）
- `testgen-agent.testCommand` (string): テスト実行コマンド（デフォルト: `npm test`）
  - 例: `pnpm test` / `bun test` / `pytest` / `go test ./...` などプロジェクトごとに変更可能にする
  - 失敗時もレポートは保存し、exit code を明記する
...
- （中止）Cursor API (`GET /v0/models`) からモデルリスト動的取得
- `git/worktreeManager.ts`: git worktree サンドボックス（任意）
- （要件）テスト観点表の事前生成＋保存（タイムスタンプ付き）
- （要件）生成後テスト実行＋実行レポート保存（タイムスタンプ付き）
...
モデルリストの動的取得（Cursor API `GET /v0/models`）は、当面スコープ外とする。

代替方針:

- **設定優先**: `testgen-agent.defaultModel` / `testgen-agent.customModels` を候補として使用
- **未設定フォールバック**: `defaultModel` が未設定の場合は **cursor-agent の自動選択**に委ねる
