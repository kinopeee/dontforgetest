...
todos:
  ...
  - id: phase2-working-tree
    ...
    status: completed
  - id: phase2-snapshot
    content: 開始時スナップショット取得（core/snapshot.ts）
    status: completed
  - id: phase3-statusbar
    content: ステータスバー進捗表示（ui/statusBar.ts）
    status: completed
  - id: phase3-quickpick
    content: QuickPickでソース/モデル選択UI（ui/quickPick.ts）★Webviewより先
    status: completed
  - id: phase3-model-settings
    content: モデル設定（設定画面 + customModelsフォールバック）
    status: completed
  - id: phase3-diff-preview
    content: 変更プレビュー/適用/ロールバック（apply/diffPreview.ts, patchApplier.ts）
    status: completed
  - id: phase3-batch
    content: 複数ファイル並列処理（maxParallelTasks制限）
    status: completed
  - id: phase4-webview-panel
    content: "[後回し] TestGen操作パネル（WebviewView）"
    status: pending
  ...
  - id: phase4-artifact-perspective-table
    content: "[要件] テスト生成前にテスト観点表を生成してファイル保存（タイムスタンプ付きで新規保存）"
    status: completed
  - id: phase4-artifact-test-exec-report
    content: "[要件] テスト生成後にテストを実行し、テスト実行レポートをファイル保存（タイムスタンプ付きで新規保存）"
    status: completed
