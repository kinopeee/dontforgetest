...
export class SettingsPanelViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  private async handleMessage(msg: WebviewMessage): Promise<void> {
    ...
    if (msg.type === 'setAgentProvider') {
      ...
      if (agentProvider !== 'cursorAgent' && agentProvider !== 'claudeCode' && agentProvider !== 'geminiCli' && agentProvider !== 'codexCli' && agentProvider !== 'copilotCli') {
        return;
      }
  ...
  private buildHtml(): string {
    ...
    return [
      ...
      `          <option value="copilotCli">Copilot CLI</option>`,
      '        </select>',
      '      </div>',
      '    </div>',
      '  </div>',
