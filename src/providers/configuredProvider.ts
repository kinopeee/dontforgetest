...
import { ClineCliProvider } from './clineCliProvider';

/**
 * エージェントプロバイダーの識別子。
 */
export type AgentProviderId = 'cursorAgent' | 'claudeCode' | 'geminiCli' | 'codexCli' | 'clineCli';
...
export function getAgentProviderId(): AgentProviderId {
  ...
  if (trimmed === 'clineCli') return 'clineCli';
  return 'cursorAgent';
}
...
export function createAgentProviderById(id: AgentProviderId): AgentProvider {
  ...
  if (id === 'clineCli') return new ClineCliProvider();
